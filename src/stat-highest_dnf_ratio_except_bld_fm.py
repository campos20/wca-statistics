...
def most_dnf_ratio():
	...
	with open("WCA_export/WCA_export_Results.tsv") as tsvfile:
		...
		prev = None
		count = 0		
		for ratio, name, country, dnf, attempt, wca_id in sorted(zip(ratios, out_names, out_countries, out_dnf, out_attempt, out_ids))[::-1]:
			count += 1
		...
		out["table"] = table
		out["title"] = "Highest DNF ratio excluding BLD and FMC events"
