...
def compare_results(ev1, ev2):
	...
	for diff, wca_id, name, result1, comp1, result2, comp2 in sorted(zip(out_diffs, out_ids, out_names, out_ev1_results, out_competition_ev1, out_ev2_results, out_competition_ev2))[::-1]:
		...
		table.append([pos, diff, html_link_format(name, link), result1, get_competition_html_link(comp1), result2, get_competition_html_link(comp2)])
	...
	out["table"] = table
	out["labels"] = ["#", "Days", "Name", "First result %s"%ev1, "Competition", "First result %s"%ev2, "Competition"]
	return out
